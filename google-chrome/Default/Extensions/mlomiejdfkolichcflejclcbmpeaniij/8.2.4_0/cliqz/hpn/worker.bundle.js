    ...
    generateKey() {
      const promise = new Promise((resolve, reject) => {
        _crypto2.default.subtle.generateKey({
          ...
          return returnData;
        }).then(keys => (0, _httpWorker2.default)(this.csm.USER_REG).post(JSON.stringify({ pk: keys.publicKeyB64 }))).then(() => resolve({ status: true, privateKey: this.privateKey, publicKey: this.publicKey })).catch(e => reject({ status: e.message }));
      });
      return promise;
    }
  }
